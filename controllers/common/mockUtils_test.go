...
func (m *mockRestClient) Post(url string, reason string, body interface{}) (*http.Response, error) {
	...
	} else if m.url == "StatusUnauthorized" {
		return &http.Response{StatusCode: 401}, nil
	} else if m.url == "StatusAccepted" {
		return &http.Response{Header: http.Header{"Location": []string{"/taskmon/statusOK"}}, StatusCode: 202}, nil
	}
	return nil, nil
}
func (m *mockRestClient) Get(url string, reason string) (map[string]interface{}, int, error) {
	if m.url == "success_case" {
		return map[string]interface{}{}, 200, nil
	} else if m.url == "error_case" {
		return nil, 400, errors.New("fakeErr")
	} else if m.url == "StatusNotFound_case" {
		return nil, 404, nil
	} else if url == "/taskmon/statusOK" {
		return map[string]interface{}{}, 200, nil
	} else if url == "/taskmon/StatusCreated" {
		return map[string]interface{}{}, 201, nil
	} else if url == "/taskmon/StatusNotFound" {
		return map[string]interface{}{}, 404, nil
	} else if url == "/taskmon/StatusBadRequest" {
		return map[string]interface{}{}, 400, nil
	} else if url == "/taskmon/StatusConflict" {
		return map[string]interface{}{}, 409, nil
	} else if url == "/taskmon/StatusNoContent" {
		return map[string]interface{}{}, 204, nil
	} else if url == "/taskmon/error" {
		return nil, 0, errors.New("fakeError")
	}
	return nil, 0, nil
}
func (m *mockRestClient) Patch(url string, reason string, body interface{}) (*http.Response, error) {
	if url == "/redfish/v1/Systems/commonFakeID/Bios/Settings/" {
		return &http.Response{StatusCode: 202, Header: http.Header{"Location": []string{"/taskmon/statusOK"}}}, nil
	}
	return nil, nil
}
func (m *mockRestClient) Delete(string, string) (*http.Response, error) {
	...
	} else if m.url == "StatusAccepted_case" {
		return &http.Response{Header: http.Header{"Location": []string{"/taskmon/statusOK"}}, StatusCode: 202}, nil
	}
	return nil, nil
}
